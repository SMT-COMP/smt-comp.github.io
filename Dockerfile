...
ENV VIRTUAL_ENV /opt/smtcomp-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/smtcomp/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/smtcomp/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/smtcomp/ && touch src/smtcomp/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/smtcomp/
...
ENTRYPOINT ["/opt/smtcomp-env/bin/smtcomp"]
CMD []
